...
def get_search_strings(tag):
    ...
    if re.search(r"[TIE]\d+\w?", tag):
        logging.debug(f"Tag {tag} is type transmitter/indicator/emitter, calibration required")
    ...
    # if tag is not correct type, return not applicable
        ...
        logging.debug(f"Tag {tag} is not type transmitter/indicator/emitter, calibration NOT required")
